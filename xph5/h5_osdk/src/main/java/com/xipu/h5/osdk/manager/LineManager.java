...
public class LineManager {
                    ...
                    if (mLineApiClient.verifyToken().isSuccess()) {
                        ...
                        mActivity.runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                H5OSDK.getInstance().mLoginMap.put("third_openid", profile.getUserId());
                                H5OSDK.getInstance().mLoginMap.put("third_type", OLoginTypeUtils.LINE);
        ...
        if (requestCode == OLoginCodeUtils.LINE_SIGN_IN) {
            ...
            try {
                ...
                switch (result.getResponseCode()) {
                    case SUCCESS:
                        H5OSDK.getInstance().mLoginMap = new HashMap<>();
                        H5OSDK.getInstance().mLoginMap.put("third_openid", result.getLineProfile().getUserId());
                        H5OSDK.getInstance().mLoginMap.put("third_type", OLoginTypeUtils.LINE);
                        H5OSDK.getInstance().mLoginMap.put("third_ext", "{" +
                    ...
                    default:
                        ...
                        if(mIsBindAccount){
                            ...
                            H5OSDK.getInstance().onThirdLoginFailure(OLoginTypeUtils.LINE,SOCommonUtil.getRes4LocaleStr(mActivity, "o_line_login_error")+": "+result.getErrorData().getMessage());
                        }
                        break;
                }
            } catch (Exception e) {
            ...
            // 分享成功
            H5OSDK.getInstance().onShareSuccess(OSharePlatform.LINE);
        }
    }
    ...
    public void shareLine(String localUrl, String text, String type) throws Exception {
        ...
        if (!OSDKUtils.getInstance().checkApkExist(mActivity, linePackageName)) {
            H5OSDK.getInstance().onShareFailure(OSharePlatform.LINE,SOCommonUtil.getRes4LocaleStr(mActivity, "o_line_no_install"));
            return;
        }
