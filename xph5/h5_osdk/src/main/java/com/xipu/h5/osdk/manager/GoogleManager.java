...
public class GoogleManager implements PurchasesUpdatedListener {
                ...
                for (String order : mSendPaySuccessOrderNoList.keySet()) {
                    try {
                        ...
                        if (orderMap.get("purchaseToken").equals(purchaseToken)) {
                            ...
                            H5OSDK.getInstance().onPaySuccess(orderMap,true);
    ...
    private BillingClientStateListener mBillingClientStateListener = new BillingClientStateListener() {
        ...
        public void onBillingServiceDisconnected() {
            ...
            if (mConnectCount >= 3) {
                SOToastUtil.showShort("Google Pay ServiceDisconnected");
                return;
            }
            ++mConnectCount;
     ...
     * 初始化 google 支付
     */
    public void initGooglePay(boolean isQueryProductID, boolean isQueryCachePurchase) {
        Log.d(OSDKUtils.TAG, "initGooglePay()");
        this.mIsQueryProductID = isQueryProductID;
        this.mIsQueryCachePurchase = isQueryCachePurchase;
        try {
    ...
    private void updateUI(GoogleSignInAccount account, String errorMsg) {
        ...
        if (account != null) {
            ...
            H5OSDK.getInstance().mLoginMap = new HashMap<>();
            H5OSDK.getInstance().mLoginMap.put("third_openid", account.getId());
            H5OSDK.getInstance().mLoginMap.put("third_type", OLoginTypeUtils.GOOGLE);
            H5OSDK.getInstance().mLoginMap.put("third_ext", "{" +
                    "\"account\":\"" + account.toJson() +
                    "\"}");
            if (mIsBindAccount) {
                H5OSDK.getInstance().onBindThirdAccount(H5OSDK.getInstance().mLoginMap);
            } else {
                H5OSDK.getInstance().onThirdLoginSuccess(H5OSDK.getInstance().mLoginMap);
            }
        } else {
            if (mIsBindAccount) {
                SOToastUtil.showShort(SOCommonUtil.getRes4LocaleStr(mActivity, "o_google_bind_onError"));
            } else {
                H5OSDK.getInstance().onThirdLoginFailure(OLoginTypeUtils.GOOGLE,SOCommonUtil.getRes4LocaleStr(mActivity, "o_google_login_error")+ ": " + errorMsg);
            }
        }
    }
    ...
    public void onPurchasesUpdated(BillingResult billingResult, @Nullable List<Purchase> purchases) {
        ...
        if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK && purchases != null) {
            ...
            H5OSDK.getInstance().onPayCancel();
        } else if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.ITEM_ALREADY_OWNED) { // 存在未确认商品
            queryPurchase();
        } else {
            // Handle any other error codes.
            H5OSDK.getInstance().onPayFailure(SOCommonUtil.getRes4LocaleStr(mActivity, "o_google_pay_error"));
        }
    }
            ...
            mBillingClient.querySkuDetailsAsync(params.build(), new SkuDetailsResponseListener() {
                ...
                public void onSkuDetailsResponse(BillingResult billingResult,
                    ...
                    if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK && skuDetailsList != null) {
                        ...
                        H5OSDK.getInstance().onPayFailure(SOCommonUtil.getRes4LocaleStr(mActivity, "o_google_query_product_exception"));
                    }
                }
            });
        } catch (Exception e) {
                ...
                for (String order : mGoogleOrderNoList.keySet()) {
                    try {
                        ...
                        if (orderMap.get("purchaseToken").equals(purchase.getPurchaseToken()) &&
                            ...
                            isMatchOrder = true;
                            H5OSDK.getInstance().onPaySuccess(setSendPayParam(purchase), false);
    ...
    private void googlePay() {
        if (!isFeatureSupported(BillingClient.FeatureType.IN_APP_ITEMS_ON_VR)) {
            H5OSDK.getInstance().onPayFailure(SOCommonUtil.getRes4LocaleStr(mActivity, "o_payment_is_not_supported_in_the_current_environment"));
            return;
        }
        ...
        ThreadManager.getInstance().getCachePoolThread().execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    H5OSDK.getInstance().onPayFailure(e.getMessage());
                }
            }
        });
    }
    ...
    private void handlePurchase(Purchase purchase) {
        ...
        try {
            ...
            H5OSDK.getInstance().onPayFailure(SOCommonUtil.getRes4LocaleStr(mActivity, "o_google_confirms_abnormal_payment"));
        }
    }

    /**
     * setConsumeFailed  report google param
     *
     * @param purchaseToken
     * @return
     */
    private HashMap<String, String> setConsumeFailedParam(String purchaseToken) {
        HashMap<String, String> paramsMap = new HashMap<>();
        try {
            paramsMap.put("token", purchaseToken);
        } catch (Exception e) {
            e.printStackTrace();
            Log.d(OSDKUtils.TAG, "setConsumeFailedParam() Exception: " + e.getMessage());
        }
        return paramsMap;
    }

    /**
     * set  report google param
     *
     * @param purchase
     * @return
     */
    private HashMap<String, String> setReportGoogleParam(Purchase purchase, String mOut_orderNo1) {
        HashMap<String, String> paramsMap = new HashMap<>();
        try {
            paramsMap.put("purchase_time", DateUtils.stampToDate(purchase.getPurchaseTime()));
            paramsMap.put("package_name", purchase.getPackageName());
            paramsMap.put("token", purchase.getPurchaseToken());
            if (!TextUtils.isEmpty(mOut_orderNo1)) {
                paramsMap.put("out_trade_no", mOut_orderNo1);
            }
            paramsMap.put("trade_no", purchase.getOrderId());
            paramsMap.put("product_id", purchase.getSku());
        } catch (Exception e) {
            e.printStackTrace();
            Log.d(OSDKUtils.TAG, "setReportGoogleParam() Exception: " + e.getMessage());
        }
